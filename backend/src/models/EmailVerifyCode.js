...
const createCode = async ({ user_id, email, code, expiresInMinutes = 10 }) => {
    ...
    const [result] = await db.execute(
        `INSERT INTO email_verify_codes (user_id, email, code_hash, attempts_left, expires_at)
         VALUES (?, ?, ?, 5, DATE_ADD(NOW(), INTERVAL ? MINUTE))`,
        [uid, email.trim().toLowerCase(), hashCode(code), Number(expiresInMinutes)]
    );
...
const getActiveCode = async (userId) => {
    ...
    const [rows] = await db.execute(
        `SELECT id, user_id, email, code_hash, attempts_left, expires_at
